...
export default async (src: string, isOwn: boolean) => {
  const color = await getAppendixColorFromImage(src, isOwn);
  const svg = (isOwn ? SAMPLE_OWN : SAMPLE_NOT_OWN).replace('{FILL}', color);
